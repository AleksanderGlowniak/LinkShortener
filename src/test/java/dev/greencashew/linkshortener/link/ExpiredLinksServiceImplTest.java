...
class ExpiredLinksServiceImplTest {
    ...
    void shouldNotRemoveNonExpiredLinks(){
        // given
        final LocalDate today = LocalDate.of(2022, 1, 1);
        LinkEntity nonexpiredLink1 = LINK_ENTITY_BUILDER.id("id1").expirationDate(today.minusDays(20)).build();
        LinkEntity nonexpiredLink2 = LINK_ENTITY_BUILDER.id("id2").expirationDate(today.minusDays(50)).build();
        LinkEntity nonexpiredLink3 = LINK_ENTITY_BUILDER.id("id3").expirationDate(today.minusDays(1800)).build();
